...
#'  
#' @export
rhInvertScore <- function(age_vals,early,mid,late,method,scoringMethod)
{
        ...
        if(scoringMethod == "max-min"){
                Score.Early <- max(age_valsMid) - min(age_valsEarly)
                Score.Late <- max(age_valsMid) - min(age_valsLate) 
        }
